...
Table~\ref{tab:main_results_unsw} presents the more challenging UNSW-NB15 results. Here, the ranking changes substantially. The original Bi-ARL remains highly unstable, whereas the route-C models are clearly stronger. In particular, BiAT-FTTransformer improves over BiAT-MLP by nearly 2.5 F1 points and reduces mean FPR by almost ten points. It remains behind LightGBM on overall F1, but its mean FPR is already lower than XGBoost and much lower than the other neural baselines.
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccc}
        ...
        BiAT-FTTransformer & 92.06\% & 85.98\% & 88.91\% & 18.41\% \\
...
\textbf{Cross-dataset interpretation}: the current Bi-ARL implementation should not be presented as a state-of-the-art IDS. The more defensible positive claim is narrower: route C substantially improves over the original RL formulation, and BiAT-FTTransformer becomes the strongest neural detector on UNSW-NB15. Although modern boosted-tree baselines remain the strongest overall, the best route-C model now achieves a lower mean FPR than XGBoost while staying within three F1 points of LightGBM.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/generated/main_results/main_results_unsw_nb15.pdf}
    \caption{Main-result visualization on UNSW-NB15. Route C substantially narrows the gap, with BiAT-FTTransformer becoming the strongest neural model and achieving a lower FPR than XGBoost, although LightGBM still leads overall.}
    \label{fig:main_unsw}
\end{figure}

...
- 它在 `UNSW-NB15` 上已经取得当前最低 `FPR`
- 它在 `CIC-IDS2017-random` 上只有小幅收益，但迁移最优 `UNSW` 配置后仍维持了正增益
...
## 13. 最优 UNSW 配置需要迁移到第二个现代数据集验证

### 问题

`UNSW-NB15` 上最优 `FT` 配置已经推进到：

- `F1 = 89.51%`
- `FPR = 14.97%`

但这还不能自动推出它在第二个现代公开基准上也同样有效。
...
固定当前最优 `UNSW` 配置，直接迁移到：

- `CIC-IDS2017-random`

新增脚本：

- `scripts/Run-FtTransferToCic.ps1`
...
下一步不继续盲目扩 `UNSW` sweep，而是优先回答一个更重要的问题：

- 这组最优 `FT` 配置能否把 `CIC-IDS2017-random` 也再往前推一点

### 最新结果

迁移验证已经完成，结果为：

- 旧 `BiAT-FTTransformer`
  - `F1 = 86.88%`
  - `FPR = 6.82%`
- 迁移最优 `UNSW` 配置后
  - `F1 = 87.09%`
...
- 最优 `UNSW` 配置并没有在 `CIC-IDS2017-random` 上退化
- 但提升幅度很有限
- 因此这组优化更像“可迁移的小幅收益”，而不是“让第二个现代数据集也出现明显跃升”
